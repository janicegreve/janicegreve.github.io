...
exports.generate3d = async () => {
  for (const lang of ['en', 'da']) {
    ...
    for (const book of booksToProcess) {
      const frontInputPath = book.status
        ? path.join(__dirname, `../src/assets/generated/${lang}/${book.id}.ribbon.png`)
        : path.join(__dirname, `../src/assets/${book.coverRaw ?? book.id}.png`);
      ...
      const outputPath = path.join(__dirname, `../src/assets/generated/${lang}/${book.id}.3d.png`);
      ...
      try {
        ...
        console.log(`✅ [${lang.toUpperCase()}] Generated: ${book.id}.3d.png`);
      } catch (err) {
